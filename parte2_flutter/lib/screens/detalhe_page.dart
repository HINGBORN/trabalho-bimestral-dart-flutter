        ...
        child: Column(
          ...
          children: [
            Image.network(
              ...
              fit: BoxFit.cover,
              // Proteção contra link quebrado ou falta de internet:
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  Text(peca.nome, style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
                  const SizedBox(height: 8),
                  Text(peca.fabricante, style: const TextStyle(fontSize: 18, color: Colors.grey)),
                  const Divider(height: 32),
                  _buildDetailRow(Icons.attach_money, 'Preço Unitário', 'R\$ ${peca.preco.toStringAsFixed(2)}'),
                  const SizedBox(height: 16),
                  _buildDetailRow(Icons.inventory, 'Quantidade em Estoque', '${peca.quantidade} un.'),
                  
                  if (peca is PecaPerformance) ...[
                    const SizedBox(height: 16),
                    _buildDetailRow(Icons.speed, 'Ganho Estimado', '+${(peca as PecaPerformance).ganhoCavalos}cv'),
                    const SizedBox(height: 16),
                    _buildDetailRow(Icons.science, 'Material', (peca as PecaPerformance).material),
                  ]
                ],
              ),
            ),
